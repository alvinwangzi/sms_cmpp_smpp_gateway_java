...
public class DeliveryReceipt {
	...
	static public DeliveryReceipt parseShortMessage(String shortMessage, DateTimeZone zone) throws RuntimeException {
	...
	static public DeliveryReceipt parseShortMessage(String shortMessage, DateTimeZone zone, boolean checkMissingFields)
		...
		if (checkMissingFields) {
			if (StringUtil.isEmpty(dlr.messageId)) {
				throw new RuntimeException("Unable to find [id] field or empty value in delivery receipt message");
			}

			if (dlr.submitCount < 0) {
				throw new RuntimeException("Unable to find [sub] field or empty value in delivery receipt message");
			}

			if (dlr.deliveredCount < 0) {
				throw new RuntimeException("Unable to find [dlvrd] field or empty value in delivery receipt message");
			}
			...
			if (dlr.state < 0) {
				throw new RuntimeException("Unable to find [stat] field or empty value in delivery receipt message");
			}

			if (StringUtil.isEmpty(dlr.rawErrorCode) && dlr.errorCode < 0) {
				throw new RuntimeException("Unable to find [err] field or empty value in delivery receipt message");
			}
		}
	...
	static public byte toState(String stateText) {
		...
		if (stateText.equalsIgnoreCase("DELIVRD") || stateText.equalsIgnoreCase("DELIVERED")) {
