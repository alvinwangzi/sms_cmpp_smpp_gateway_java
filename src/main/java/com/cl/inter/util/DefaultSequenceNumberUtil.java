...
public class DefaultSequenceNumberUtil {
	public static byte[] sequenceN2Bytes(SequenceNumber sn) {
		...
		long t = Long.parseLong(String.format("%1$02d%2$02d%3$02d%4$02d%5$02d", sn.getMonth(), sn.getDay(),
				sn.getHour(), sn.getMinutes(), sn.getSeconds()));
	...
	private final static AtomicLong sequenceId = new AtomicLong(Math.abs(RandomUtils.nextInt(0, 255)));
}
